...
const Withdrawals = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await axios.post("/users/withdrawals", withdrawalData);
      Swal.fire(
        "Success!",
    ...
    <div className="max-w-lg mx-auto mt-10 bg-white rounded-lg shadow-md p-6">
      ...
      <div className="mb-6 text-center">
        <p className="font-semibold">
          Your Current Coin:{" "}
          <span className="text-blue-600">{userData?.coins || 0}</span>
        </p>
        <p className="font-semibold">
          Equivalent Dollar:{" "}
          <span className="text-green-600">
            ${(userData?.coins / 20).toFixed(2)}
          </span>
        </p>
      </div>
          ...
          <div>
            ...
            {errors.coin && (
              <p className="text-sm text-red-500">
                Enter a valid coin amount (min 200)
              </p>
            )}
          </div>
